...
There are other apps that perform a similar function, but I really wanted something Mac native that lives in the menu bar for easy access. Also, I wanted all data stored in local markdown files, so that I could read/edit/add them in Obsidian. I personally keep all of my Life in Weeks notes in a folder in my Obsidian vault. 

![life in weeks - sample screen shot](./sample-screen.png)
...
_(If you already know what_ `_git clone_` _is, feel free to use that instead.)_
...
On first launch the app asks for a few configuration properties:

**A storage folder** — This is where all of your notes will be stored. By default, a folder of its own (`~/Documents/LifeInWeeks` is offered), or you can change this to whatever you like. If you wish to view, edit and add notes from within Obsidian, set this as a path inside an existing Obsidian vault. Only this folder is ever read or watched, never the rest of a vault.

**Your name, birth date, and the age to draw through** (90 by default). The app projects life through age 90 by default. There are [various](https://www.livingto100.com/) [calculators](https://apps.goldensoncenter.uconn.edu/HLEC/) available that you can use to obtain a personalized estimate.
...
MIT — see [LICENSE](https://github.com/ballmarian/life-in-weeks/blob/main/LICENSE).